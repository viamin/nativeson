...
class NativesonTest < ActiveSupport::TestCase
  ...
  test "fetch_json_by_query_hash with columns from joined table" do
    query_hash = query_defaults.merge({
      ...
      joins: [{klass: "UserProfile", on: "users.id", foreign_on: "user_profiles.user_id"}],
      key: "users"
    })
    expected_json = <<~JSON
  ...
  test "fetch_json_by_query_hash with columns from coalesced columns" do
    query_hash = query_defaults.merge({
      klass: "User",
      columns: ["name", "email", {coalesce: ["col_string", "user_profiles.name"], as: "string"}],
      joins: [{klass: "UserProfile", on: "users.id", foreign_on: "user_profiles.user_id"}],
      key: "users"
    })
    expected_json = <<~JSON
  ...
  test "fetch_json_by_query_hash with deeply nested associations" do
    query_hash = query_defaults.merge({
      klass: "User",
      columns: ["name", "email", {name: "items.name", as: "item"}, {name: "item_descriptions.description", as: "description"}],
      joins: [
        {klass: "Item", on: "users.id", foreign_on: "items.user_id"},
        {klass: "ItemDescription", on: "items.id", foreign_on: "item_descriptions.item_id"}
      ],
      key: "users"
    })
    expected_json = <<~JSON
