module TestHelpers
  ...
  def self.compare_ar_instance_to_nativeson_hash(ar_instance, nativeson_hash, columns_to_ignore = %w(updated_at created_at))
    ...
    (ar_instance.attributes.keys + nativeson_hash.keys).uniq.each do |key|
      ...
      unless equal
        highlight_text("Model = #{ar_instance.class} | ID = #{ar_instance.try(:id)} | nativeson_hash missing key = #{key}")
        break
      end
      equal = equal && ar_instance.attributes.key?(key)
      unless equal
        highlight_text("Model = #{ar_instance.class} | ID = #{ar_instance.try(:id)} | ar_instance missing key = #{key}")
        break
      end
      ...
      unless equal
        highlight_text("Model = #{ar_instance.class} | ID = #{ar_instance.try(:id)}")
        highlight_text("key = #{key} | ar_val = #{ar_val} | nativeson_val = #{nativeson_val}")
        break
      end
    end
    equal
  ...
  def self.random_attributes(ar_instance, ignore_attributes: %w(created_at updated_at),
                             mandatory_attributes: %w(id), size_limit: 1)
    output_attributes = {}
    count = 0
